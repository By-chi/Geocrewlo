...
func _ready() -> void:
	...
	GameData.score.resize(half_population)
	GameData.mortality_database.resize(half_population)
	var arr: Array[int]
	arr.resize(half_population)
	arr.fill(0)
	for i in half_population:
		GameData.score[i]=arr.duplicate()
		GameData.mortality_database[i]=arr.duplicate()
	...
	for i in MapData.start_location[arg["Map"]].size():
		...
		for j in half_population:
			var x := lerpf(
				MapData.start_location[arg["Map"]][i].x,
				MapData.start_location[arg["Map"]][i].z,
				j / float(half_population)
			)
			var y := lerpf(
				MapData.start_location[arg["Map"]][i].y,
				MapData.start_location[arg["Map"]][i].w,
				j / float(half_population)
			)
